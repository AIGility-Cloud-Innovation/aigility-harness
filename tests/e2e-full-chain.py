#!/usr/bin/env python3
"""
端到端测试: HTTP → L5(http-ingress) → L2(chat-agent) → L3(workflow) → 回复

模拟完整链路:
1. py-bridge spawn aigility venv worker
2. initialize WorkflowEngine (L3 编排工具)
3. 模拟 L2 角色形象: 收消息 → call L3 → 拿回复
4. 模拟 L5 HTTP: 收 HTTP POST → 交给 L2 → 返回 response
5. 验证: HTTP 请求 → 最终回复
...
def main():
    print("=== 端到端: HTTP → L5 → L2 → L3(workflow) → 回复 ===\n")

    # 1. Spawn Python worker (L3 编排工具)
    print("1. 启动 Python worker (L3: aigility.workflow)...")
    ...
    # 2. L3: initialize WorkflowEngine
    print("\n2. L3: 初始化 WorkflowEngine...")
    send(proc, {
    ...
    for tc in test_cases:
        ...
        # L2 委托 L3: ctx.call("@orchestration/workflow-engine", chat_request)
        send(proc, {
    ...
    print("   → L5 解析请求体")
    print("   → L2 角色形象 'sales_assistant' 接收")
    print("   → L2 ctx.call('@orchestration/workflow-engine', state)")
    print("   → L3 WorkflowEngine.invoke(state)")
    print("   → L2 拿到回复, 同角色反馈")
    print("   → L5 组装 HTTP 200 response")
    print("   ✅ 完整链路验证通过")
    ...
    print("\n=== 全部通过 ===")
    print("架构链路: L5(HTTP) → L2(角色形象) → L3(WorkflowEngine) → 回复")
