...
rem  start-appbase.cmd). Kills the process listening on :1231.
...
set "KILLPID="
for /f "tokens=5" %%p in ('netstat -ano ^| findstr ":1231" ^| findstr "LISTENING"') do set "KILLPID=%%p"
if not defined KILLPID (
  echo [stop-appbase] AppBase is not running ^(nothing listening on 1231^).
  exit /b 0
)
...
netstat -ano | findstr ":1231" | findstr "LISTENING" >nul 2>nul
if errorlevel 1 (
  echo [stop-appbase] stopped. Start again with: start-appbase.cmd
  exit /b 0
)
echo [stop-appbase] WARN: something is still listening on 1231.
exit /b 1
