...
>
> 状态：**已实现**（本文为设计稿留存；实现见 `packages/layer-infrastructure/src/feishu-ingress.ts`
> + 单测 `feishu-ingress.test.ts`，已从 layer-infrastructure 入口导出。
> 与设计稿的差异：service id 定为 `@infrastructure/feishu-ingress`（manifest 名 `@infrastructure/feishu`），
> 默认路由人格为 `@persona/timem-project-assistant`。）
